...
into the mix. While many of these basic concepts were familar to me
...
concurrency primitives in a high-level language such as Rust was still
interesting (and at times surprising) to me. I hope that this post can
...
and may, at times, even be able to write to arbitary regular program
...
interface typically do so by having their registers layed out such
...
** src_rust[:exports code]{VolatileCell} and =dereferencable=
...
references in Rust are marked as =dereferencable= by default.
...
optimations, or hardware-reordering of memory accesses, though. For
...
when those buffer writes have not actually been comitted to memory
yet.
