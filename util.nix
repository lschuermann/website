...
let
        ...
        pkgs.lib.foldl
          (
            ...
            if partInitMatch != null then
              {
                parts = parts // {
                  "${builtins.head partInitMatch}" = null;
                };
            ...
            else if currentPart != null then
              {
                ...
                parts = parts // {
                  "${currentPart}" =
                    if parts."${currentPart}" == null then line else parts."${currentPart}" + "\n" + line;
                };
              }
            else
  ...
  expandDate = datestr: parseRFC3339Sec "${datestr} 00:00:00-00:00";
  ...
  parseRFC3339Sec =
    ...
    let
      ...
      leapYearCorrectionDays =
        fromYear: toYear:
        let
          yearSpan = toYear - fromYear;
          yearsToFirstLeapYear = lib.mod (4 - (lib.mod fromYear 4)) 4;
          leapYearConsiderationYears = lib.max 0 (yearSpan - yearsToFirstLeapYear);
        in
        (leapYearConsiderationYears / 4) + (lib.min 1 (lib.mod leapYearConsiderationYears 4));

      stripLeadingZeroes =
        str:
        let
          strippedCharList = lib.foldl (
            acc: char: if char != "0" || acc != [ ] then acc ++ [ char ] else acc
          ) [ ] (lib.filter (char: char != "") (lib.splitString "" str));
        in
        if strippedCharList == [ ] then "0" else lib.concatStringsSep "" strippedCharList;
  ...
  formatRFC822 = p: with p; "${a}, ${d} ${b} ${Y} ${k}:${M}:${S} ${z}";
